...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="UltraDiffEdit with an OpenPose ControlNet first stage.")
    ...
    parser.add_argument("--output", default="results/controlnet_pose.png")
    parser.add_argument("--ckpt", default="stabilityai/stable-diffusion-xl-base-1.0")
    parser.add_argument("--controlnet_ckpt", default="thibaud/controlnet-openpose-sdxl-1.0")
    parser.add_argument("--pose_annotator_ckpt", default="lllyasviel/ControlNet")
    parser.add_argument("--control_source", default=None, help="Optional image used only to estimate pose.")
    ...
    parser.add_argument("--detect_resolution", type=int, default=1024)
...
def make_pose_condition(image: Image.Image, annotator_ckpt: str, detect_resolution: int, image_resolution: int) -> Image.Image:
    try:
...
def main() -> None:
    ...
    first_control = make_pose_condition(
        first_control_source,
        args.pose_annotator_ckpt,
