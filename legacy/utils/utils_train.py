...
def write_in_File(target_file_path, list_file_path):
    ...
    for file_path in list_file_path:
        print(f"{file_path} write in the {target_file_path}")
        target_file.write(f'{file_path} \n')
    target_file.close();
...
def copy_Dir2Dir(srcDir, dstDir):
    '''
...
def copydirs(from_file, to_file):
    ...
    for f in files:
        if os.path.isdir(os.path.join(from_file, f)):  # 判断是否是文件夹
            ...
            fromfile = os.path.join(from_file, f)
            tofile = os.path.join(to_file, f)
            if os.path.exists(tofile):
                fromtime = get_file_modified_time(fromfile)
                totime = get_file_modified_time(tofile)
                print("from_file:",fromtime)
                print("totime:",totime)

                if os.path.getmtime(fromfile)> os.path.getmtime(tofile):
                    print(f"copy!!: from:{fromfile}  to {tofile}")
                    # shutil.copy(fromfile, tofile)  # 拷贝文件
                    try:
                        shutil.copy(fromfile, tofile)  # 拷贝文件
                    except OSError:
                        print(f"OSError: {fromfile}")
                else:
                    print(f"the modified time is the same jump!!")
...
if __name__ == '__main__':
    from_file = "E:\\Longlongaaago3"
    to_file = "F:\\Longlongaaago3"
