...
def blend(target: Tensor, source: Tensor, mask: Tensor, corner_coord: Tensor, mix_gradients: bool,
          ...
          integration_mode: str = 'origin',
          restore_detail = False,
          data_range=[-1,1]) -> Tensor:
    ...
    if restore_detail:  # restore_detail
        laplacian_target = torch.sum(torch.stack([torch.gradient(grad, dim=grad_dim)[0]
                                                  for grad, grad_dim in zip(target_grads, chosen_dimensions)]), dim=0)
        init_blended_target = torch.fft.ifftn(torch.fft.fftn(laplacian_target, dim=chosen_dimensions) * green_function,
                                              dim=chosen_dimensions)

        integration_constant_target = init_blended_target[tuple([slice(1) if i in chosen_dimensions else slice(None)
                                                                 for i in range(num_dims)])]
        # Leave out padding + border, to avoid artefacts
        inner_blended_target = init_blended_target[
            tuple([slice(PAD_AMOUNT + 1, -PAD_AMOUNT - 1) if i in chosen_dimensions else slice(None)
                   for i in range(num_dims)])]
        res_target = target.clone()
        res_target[res_indices] = torch.real(inner_blended_target - integration_constant_target)
        # res_target = res_target.clip(0, 1)
        res_target = res_target.clip(data_range[0], data_range[1])
        # residuals = (target - res_target) * (1 - mask)
        residuals = (target - res_target)
        # result = (result + residuals).clip(0, 1)
        result = (result + residuals).clip(data_range[0], data_range[1])
