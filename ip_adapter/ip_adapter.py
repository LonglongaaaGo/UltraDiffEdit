...
class IPAdapter:
    ...
    def get_image_embeds(self, pil_image=None, clip_image_embeds=None):
        if pil_image is not None:
            ...
            clip_image_embeds = self.image_encoder(clip_image.to(self.device, dtype=self.dtype)).image_embeds
        else:
...
class IPAdapterXL_ultra(IPAdapter):
    """SDXL + controlnet + inpainting  ultra version"""
    ...
    def generate(
        ...
        num_prompts = 1 if isinstance(image, Image.Image) else len(image)
...
class IPAdapterXL_ultra_inpaint(IPAdapter):
    """SDXL  + inpaintingultra version"""
