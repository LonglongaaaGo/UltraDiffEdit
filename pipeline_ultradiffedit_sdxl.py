...
class StableAnysizeInpaintPipeline(
    ...
    def __call__(
        ...
        if scale_num<=1: run_list = []
            ...
            if run_stage == "three":
                ...
                #
                half_num = int(scale_num / 2)
                if half_num % 2 != 0: half_num += 1  # 或 half_num -= 1，根据需要选择最近的偶数
                if int(half_num)!= scale_num and int(half_num)>1:
                    run_list = [int(half_num)]+ run_list

            elif run_stage == "S":
                # run_list = [current_scale_num for current_scale_num in range(2, scale_num + 1)]
                run_list = [current_scale_num for current_scale_num in range(2, scale_num + 1) if current_scale_num % 2 == 0]
        ...
        # for current_scale_num in run_list:
            ...
            editing_latents_local = self.multi_patch_encoding(latents, pre_img, device, current_height, current_width,
                             scale_masked_imgs, scale_masks, current_scale_num, output_type, show_image,
                             latents.dtype, generator,sigma,file_name=file_name)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    jitter_range = (self.unet.config.sample_size*ug_scale - up_stride) // 4
                    latents_ = F.pad(latents, (jitter_range, jitter_range, jitter_range, jitter_range), 'constant', 0)
    ...
    def refine_editing(
        ...
        if scale_num <= 1:
            ...
            if run_stage == "three":
                # run_list = [int(scale_num / 2)+1] + run_list
                if int(scale_num / 2) != scale_num and int(scale_num / 2) > 1:
                    run_list = [int(scale_num / 2)] + run_list

            elif run_stage == "S":
                # run_list = [current_scale_num for current_scale_num in range(2, scale_num + 1)]
                run_list = [current_scale_num for current_scale_num in range(2, scale_num + 1) if
                            current_scale_num % 2 == 0]
        for idx_,current_scale_num in enumerate(run_list):
            if self.lowvram:
                latents = latents.to(device)
                self.unet.to(device)
